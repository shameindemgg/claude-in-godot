...
func set_cell(params: Dictionary) -> Dictionary:
	...
	ur.add_do_method(node.set_cell.bind(cell_pos, source_id, atlas))
	ur.add_undo_method(node.set_cell.bind(cell_pos, node.get_cell_source_id(cell_pos), node.get_cell_atlas_coords(cell_pos)))
	ur.commit_action()
	...
	if node is TileMapLayer:
		...
		for x in range(rx, rx + rw):
			for y in range(ry, ry + rh):
				...
				ur.add_do_method(node.set_cell.bind(pos, source_id, atlas))
				ur.add_undo_method(node.set_cell.bind(pos, node.get_cell_source_id(pos), node.get_cell_atlas_coords(pos)))
		...
		for x in range(rx, rx + rw):
			for y in range(ry, ry + rh):
				var pos := Vector2i(x, y)
				ur.add_do_method(node.set_cell.bind(layer, pos, source_id, atlas))
				ur.add_undo_method(node.set_cell.bind(layer, pos, node.get_cell_source_id(layer, pos), node.get_cell_atlas_coords(layer, pos)))
...
func get_cell(params: Dictionary) -> Dictionary:
	...
	if node is TileMapLayer:
		var src := node.get_cell_source_id(cell_pos)
		var atlas := node.get_cell_atlas_coords(cell_pos)
		var alt := node.get_cell_alternative_tile(cell_pos)
		return {
			"success": true,
			"node_path": node_path,
			"coords": [cell_pos.x, cell_pos.y],
		...
		var src := node.get_cell_source_id(layer, cell_pos)
		var atlas := node.get_cell_atlas_coords(layer, cell_pos)
		var alt := node.get_cell_alternative_tile(layer, cell_pos)
		return {
			"success": true,
			"node_path": node_path,
			"coords": [cell_pos.x, cell_pos.y],
...
func clear(params: Dictionary) -> Dictionary:
	...
	if node is TileMapLayer:
		...
		var used := node.get_used_cells(layer)
		for pos in used:
			ur.add_undo_method(node.set_cell.bind(layer, pos, node.get_cell_source_id(layer, pos), node.get_cell_atlas_coords(layer, pos)))
		ur.add_do_method(node.clear_layer.bind(layer))
		ur.commit_action()
...
func get_info(params: Dictionary) -> Dictionary:
	...
	if node is TileMapLayer:
		...
		var layer_count := node.get_layers_count()
		var layers := []
		for i in range(layer_count):
			var used := node.get_used_cells(i)
			layers.append({
				"index": i,
...
func _set_cell_tilemap(tilemap: TileMap, coords: Array, source_id: int, atlas_coords: Array, layer: int) -> Dictionary:
	...
	ur.add_do_method(tilemap.set_cell.bind(layer, cell_pos, source_id, atlas))
	ur.add_undo_method(tilemap.set_cell.bind(layer, cell_pos, tilemap.get_cell_source_id(layer, cell_pos), tilemap.get_cell_atlas_coords(layer, cell_pos)))
	ur.commit_action()
